...
class RealisticTrendingCalculator:
    ...
    async def fetch_character_media_data(self, character):
        """Fetcha i dati della serie associata al personaggio"""
        try:
            anilist_id = character['anilistId']
            
            # Prima recuperiamo il personaggio per ottenere le serie associate
            character_data = await self.character_fetcher.fetch_character_by_id(anilist_id)
            
            if not character_data or not character_data.media_appearances:
                logger.warning(f"⚠️  Nessuna serie trovata per {character['name']}")
                return None
            
            # Prendiamo la prima serie (la più popolare)
            first_media = character_data.media_appearances[0]
            media_id = first_media.anilist_id
            
            # Fetchiamo i dati completi della serie
            media_data = await self.media_fetcher.fetch_media_by_id(media_id)
            
            if media_data:
                logger.info(f"✅ Dati media per {character['name']}: {media_data.title_romaji}")
                return media_data
            else:
                logger.warning(f"⚠️  Dati media non trovati per {character['name']}")
                return None
                
        except Exception as e:
            logger.error(f"❌ Errore fetch media per {character['name']}: {e}")
            return None
    ...
    def calculate_realistic_trending_score(self, character, media_data):
        ...
        CON MOLTIPLICATORI MOLTO PIÙ BASSI E REALISTICI
        """
        
        # Base score dai favourites del personaggio (normalizzato)
        base_score = min(character['favourites'] / 100, 100)  # Max 100
        
        # Se non abbiamo dati media, restituiamo solo il base score
        if not media_data:
            logger.warning(f"⚠️  Nessun dato media per {character['name']}, usando solo base score")
            return {
    ...
    async def save_trending_to_db(self, character, trending_data):
        ...
        try:
            ...
            'v2.0_realistic',                          # algorithm_version
    ...
    async def process_all_characters(self):
        """Processa tutti i personaggi e calcola/salva i trending"""
        ...
        for i, character in enumerate(characters, 1):
            try:
                ...
                # 1. Fetch dati media
                media_data = await self.fetch_character_media_data(character)
                
                # 2. Calcola trending score
                trending_data = self.calculate_realistic_trending_score(character, media_data)
                
                # 3. Salva nel database
                await self.save_trending_to_db(character, trending_data)
                
                # Log risultato
                if trending_data['media_data']:
                    logger.info(f"✅ {character['name']}: {trending_data['calculated_trending_score']} "
                              f"(boost: {trending_data['total_boost_multiplier']}x, "
                              f"serie: {trending_data['media_data']['title']})")
                else:
                    logger.info(f"⚠️  {character['name']}: {trending_data['calculated_trending_score']} (solo base score)")
...
if __name__ == "__main__":
    print("🚀 Avvio calcolo trending realistici...")
    asyncio.run(main())
